...
class HIT(database.Base):
    ...
    def accept(self, reason = None, bs = False, number=0):
        if self.compensated:
            return
        ...
        for schedule in self.group.schedules:
            # print schedule
            # print schedule.award
            schedule.award(self, number)
        ...
        try:
            ...
            print "Failed to accept {0}".format(self.assignmentid)
            return
        self.accepted = True
    ...
    def warn(self, reason = "", number=0):
        if self.compensated:
            return
        reason += "\nWarning: we will start REJECTING your work soon if you do not improve your quality. Please reread the instructions."
        # if not reason:
    ...
    def reject(self, reason = ""):
        ...
        # fd = open("/home/robolab/re3_server/log.txt", 'a')
        # fd.write(str(self.hitid) + "\n")
        # fd.write(str(self.assignmentid) + "\n")
        # fd.write("com before " + str(self.compensated)+ "\n")
        # fd.write("acc before " + str(self.accepted) + "\n")
        ...
        # fd.write("com after " + str(self.compensated) + "\n")
        # fd.write("acc after " + str(self.accepted) + "\n")
        # fd.write("\n")
        # fd.flush()
        # fd.close()
