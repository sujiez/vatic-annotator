...
class compensate(Command):
    def setup(self):
        ...
        parser.add_argument("--reason", action="store")
        parser.add_argument("--default", default="defer",
            choices=["accept", "reject", "defer", "warn"])
        parser.add_argument("--limit", type=int, default = None)
        return parser

    def process(self, hit, acceptkeys, rejectkeys, warnkeys, validated, default, reason):
        if hit.validated and validated:
            ...
            hit.accept(reason, number=acceptkeys[hit.assignmentid])
        elif hit.assignmentid in warnkeys:
            reason = reason if reason else ""
            hit.warn(reason,number=warnkeys[hit.assignmentid] )
...
class invalidate(Command):
    ...
    def process_reject(self, hit, num):
        try:
            hit.reject()
        except:
            if num >= 0:
                print "Fail to reject HIT {0} {1} time in invalidate".format(hit.hitid, str(6 - num))
                self.process_reject(hit, num - 1)
            else:
                print "Give up rejecting HIT {0} in invalidate".format(hit.hitid)
    ...
    def __call__(self, args):
        ...
        if args.hit:
            rejectkeys.extend(line.strip() for line in open(args.id))

            for hit in query:
                if hit.hitid in rejectkeys:
                    self.process_reject(hit, 5)
                    pass
                if hit.compensated:
                    if not hit.accepted:
                        print "Rejected HIT {0}".format(hit.hitid)
        ...
        for hit in query:
            if args.hit and hit.hitid not in rejectkeys:
                continue
